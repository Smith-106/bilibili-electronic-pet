(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))o(s);new MutationObserver(s=>{for(const d of s)if(d.type==="childList")for(const p of d.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&o(p)}).observe(document,{childList:!0,subtree:!0});function a(s){const d={};return s.integrity&&(d.integrity=s.integrity),s.referrerPolicy&&(d.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?d.credentials="include":s.crossOrigin==="anonymous"?d.credentials="omit":d.credentials="same-origin",d}function o(s){if(s.ep)return;s.ep=!0;const d=a(s);fetch(s.href,d)}})();const ve=Object.freeze({petName:"Mochi",statusLine:"Resting near the browser edge, ready for the next ritual check-in.",loopMode:"Companion seed state",lastCheckIn:"2026-04-10 03:30",adapterLabel:"Seed state adapter",loopHint:"A shipped seed state is active until the live companion endpoint publishes richer state.",mood:{label:"Curious",note:"Ready to nudge the next companion interaction with a calm status pulse."},memoryTitle:"Short-term memory",memorySummary:"Keeps the latest ritual cues nearby so the companion can feel consistent while running on a shipped seed state.",vitals:[{label:"Energy",value:"76%"},{label:"Hunger",value:"Snack soon"},{label:"Bond",value:"Growing"},{label:"Focus",value:"Watching queue"}],recentSignals:["Last pat received 2 minutes ago.","Quiet window open for another 18 minutes.","Next nudge stays on the seed state until the live endpoint is available."],recentInteractions:[{kind:"pat",title:"Pat interaction",detail:"A calm pat kept Mochi settled on the browser ledge.",timestamp:"2026-04-10T03:28:00.000Z",source:"Seed state adapter"},{kind:"signal",title:"Status pulse",detail:"The local adapter emitted a lightweight keep-alive signal.",timestamp:"2026-04-10T03:30:00.000Z",source:"Seed state adapter"}]});function ye(e){return JSON.parse(JSON.stringify(e))}function ee(e=ve){return{async getCompanionState(){return await Promise.resolve(),ye(e)}}}async function Se(e){try{return await e.getCompanionState()}catch{return ee().getCompanionState()}}function ke(e,{reason:t,endpoint:a,legacyEndpoint:o}){const s=String(t||"backend_unavailable").trim()||"backend_unavailable",d=[a,o].filter(Boolean);return{...e,loopMode:"Degraded backend snapshot",adapterLabel:"Degraded backend snapshot",loopHint:"Backend companion state is unavailable. This labeled degraded snapshot keeps the surface explorable, but it is not live backend data.",recentSignals:[`Backend sync failed: ${s}.`,"Retry after the companion backend recovers to restore live state.",...Array.isArray(e==null?void 0:e.recentSignals)?e.recentSignals:[]],recentInteractions:[{kind:"fallback",title:"Degraded backend snapshot",detail:`The backend companion state could not be loaded from ${d.join(" -> ")||a}. Showing a degraded backend snapshot instead.`,timestamp:new Date().toISOString(),source:"Backend degraded"},...(Array.isArray(e==null?void 0:e.recentInteractions)?e.recentInteractions:[]).map(p=>({...p,source:`${(p==null?void 0:p.source)||"Seed state adapter"} · degraded snapshot`}))],degraded:!0,dataSource:"local-fallback",backendStatus:{degraded:!0,source:"backend",reason:s,endpoint:a,legacyEndpoint:o||null,retryable:!0}}}function ce(e){var t,a;try{return((a=(t=globalThis.sessionStorage)==null?void 0:t.getItem(e))==null?void 0:a.trim())||""}catch{return""}}function we(){return String(globalThis.__ADMIN_SESSION_TOKEN__||ce("admin_session_token")).trim()}function $e(){return String(globalThis.__ADMIN_API_KEY__||ce("admin_api_key")).trim()}function Ae(){const e={Accept:"application/json","Content-Type":"application/json"},t=we(),a=$e();return t&&(e["x-admin-session"]=t),a&&(e["x-api-key"]=a),e}function Ce({endpoint:e="/companion/state-v2",legacyEndpoint:t="/companion/state",actionEndpoint:a="/companion/actions",fetchImpl:o=(d=>(d=globalThis.fetch)==null?void 0:d.bind(globalThis))(),fallback:s=ee()}={}){return{async getCompanionState(){if(typeof o!="function")return s.getCompanionState();let p=!1;try{const m=await o(e,{headers:{Accept:"application/json"}});if(!m.ok&&t){p=!0;const b=await o(t,{headers:{Accept:"application/json"}});if(!b.ok)throw new Error(`companion_state_${b.status}`);const $=await b.json();if(!$||typeof $!="object")throw new Error("companion_state_invalid");return $}if(!m.ok)throw new Error(`companion_state_${m.status}`);const h=await m.json();if(!h||typeof h!="object")throw new Error("companion_state_invalid");return h}catch(m){const h=await Se(s);return ke(h,{reason:m instanceof Error?m.message:"backend_unavailable",endpoint:e,legacyEndpoint:p?t:null})}},async performAction(p,m){if(typeof o!="function")return{ok:!1,fallback:!0};const h=await o(a,{method:"POST",headers:Ae(),body:JSON.stringify({action:p,note:m})});if(!h.ok)throw new Error(`companion_action_${h.status}`);return h.json()}}}const Le=[{label:"Energy",value:"Unknown"},{label:"Hunger",value:"Unknown"},{label:"Bond",value:"Unknown"}],Te=["Local companion loop has not reported any recent signals yet."],Ee=[{kind:"signal",title:"Companion signal pending",detail:"No structured interaction timeline is available yet.",timestamp:"Pending",source:"Seed state adapter"}],q=["all","pat","feed","wake","signal","fallback"],R=["pat","feed","wake"],O=Symbol("petCompanionCleanup");function de(e){return{key:String(e+1),label:`Alt+${e+1}`}}function xe(e){const t=String(e??"").trim().toLowerCase();return t==="pat"||t==="feed"||t==="wake"||t==="signal"?t:t==="fallback"?"fallback":"signal"}function Ie(e){const t=String((e==null?void 0:e.source)??"").trim().toLowerCase(),a=String((e==null?void 0:e.title)??"").trim().toLowerCase();return t.includes("fallback")||a.includes("fallback")||t.includes("degraded")||a.includes("degraded")?"fallback":a.includes("pat")?"pat":a.includes("feed")?"feed":a.includes("wake")?"wake":"signal"}function g(e){return e==="all"?"All":e==="pat"?"Pat":e==="feed"?"Feed":e==="wake"?"Wake":e==="fallback"?"Degraded":"Signal"}function k(e){const t=String(e??"").trim().toLowerCase();return q.includes(t)?t:"all"}function K(e){return R.includes(k(e))}function Pe(e){const t=k(e);return t==="pat"?{label:"Pat note",placeholder:"Optional note for the next pat.",hint:"Describe the comfort, bond, or calming signal you want the timeline to capture. Press Ctrl+Enter to send."}:t==="feed"?{label:"Feed note",placeholder:"Optional note for the next feed.",hint:"Add snack, refill, or appetite context so the feed entry reads clearly later. Press Ctrl+Enter to send."}:t==="wake"?{label:"Wake note",placeholder:"Optional note for the next wake.",hint:"Explain the nudge or prompt that should bring the companion back into motion. Press Ctrl+Enter to send."}:{label:"Interaction note",placeholder:"Optional note for the next pat, feed, or wake.",hint:"Optional context travels into the companion timeline."}}function De(e){const t=k(e);return K(t)?null:t==="signal"?{message:"Signal entries are read-only snapshots. Pick Pat, Feed, or Wake to focus the composer on a writable action.",shortcuts:R}:t==="fallback"?{message:"Degraded-state entries describe backend recovery only. Switch to Pat, Feed, or Wake before drafting the next note.",shortcuts:R}:{message:"Notes publish through Pat, Feed, or Wake actions. Pick one to focus the composer before writing.",shortcuts:R}}function Fe(e){const t=k(e);return t==="signal"?"Signal entries are read-only. Pick Pat, Feed, or Wake before sending.":t==="fallback"?"Degraded-state entries are read-only. Pick Pat, Feed, or Wake before sending.":"Pick Pat, Feed, or Wake before sending a note."}function Me(e){const t=k(e);return t==="pat"?{label:"Suggested pat notes",templates:["Soft pat settled Mochi into a calmer loop.","Bond signal ticked upward after a gentle tap.","Comfort pass landed right on time for the next check-in."]}:t==="feed"?{label:"Suggested feed notes",templates:["Refilled snack tray and appetite stabilized.","Quick bite restored energy before the next loop window.","Treat drop landed cleanly and hunger signal eased."]}:t==="wake"?{label:"Suggested wake notes",templates:["Bright nudge reopened the interaction window.","Wake pulse brought Mochi back into active mode.","Gentle prompt resumed the browser buddy loop."]}:null}function Be(e,t,a){const o=k(e),s=String(t??"").trim();return a?{label:"Template waiting",detail:"Choose Replace, Append, or Cancel to resolve the current draft.",tone:"pending"}:s?K(o)?{label:`${g(o)} draft ready`,detail:`Will publish with the next ${g(o).toLowerCase()} action.`,tone:"ready"}:{label:"Draft waiting",detail:"Pick Pat, Feed, or Wake to send this note.",tone:"pending"}:K(o)?{label:`${g(o)} draft empty`,detail:"Type a note or pick a template to stage the next action.",tone:"idle"}:{label:"Composer idle",detail:"Select Pat, Feed, or Wake to focus the draft composer.",tone:"idle"}}function Ne(){return`
  ...
  `}function Ue(e){return e.map(t=>`
      ...
      `).join("")}function Ge(e,t="all"){const a=pe(e),o=k(t);return`
    ${We(a)}
...
${S}`:c.value=S,c.focus(),E(),C(),y(`${g(v)} template ${n==="append"?"appended to":"replaced"} draft.`)}function C(){const n=Pe(v),r=Be(v,c==null?void 0:c.value,S),u=Me(v),B=De(v),P=!!(c!=null&&c.value.trim())||!!S;x&&(x.textContent=n.label),c&&(c.placeholder=n.placeholder,c.setAttribute("data-composer-kind",k(v))),H&&(H.textContent=n.hint),F&&F.setAttribute("data-status-tone",r.tone),_&&(_.textContent=r.label),z&&(z.textContent=r.detail),D&&(D.disabled=!P),I&&(u?(I.hidden=!1,I.innerHTML=`
        ...
        `,[...I.querySelectorAll('[data-role="composer-template"]')].forEach(f=>{f.addEventListener("click",()=>{if(c){const w=f.getAttribute("data-template-value")??"",le=c.value.trim();if(le&&le!==w){S=w,C(),y("Template selected. Choose Replace, Append, or Cancel.");return}c.value=w,c.focus(),E(),C(),y(`${g(v)} template inserted into draft.`)}})})):(I.innerHTML="",I.hidden=!0)),L&&(!S||!u?(L.innerHTML="",L.hidden=!0):(L.hidden=!1,L.innerHTML=`
        ...
        `,[...L.querySelectorAll('[data-role="template-merge-action"]')].forEach(f=>{f.addEventListener("click",()=>{const w=f.getAttribute("data-merge-mode");if(w==="replace"||w==="append"){be(w);return}E(),C(),y("Template merge cancelled.")})}))),T&&(B?(T.hidden=!1,T.innerHTML=`
          <p class="composer-guide-copy">${i(B.message)}</p>
        ...
        `,[...T.querySelectorAll('[data-role="composer-shortcut"]')].forEach(f=>{f.addEventListener("click",()=>{const w=k(f.getAttribute("data-shortcut-kind"));Q(w,{announcement:`Timeline filter set to ${g(w)}.`}),c==null||c.focus()})})):(T.innerHTML="",T.hidden=!0))}function U(n){s.disabled=n,l.forEach(r=>{r.disabled=n}),c&&(c.disabled=n)}c==null||c.addEventListener("input",()=>{S&&E(),C()}),D==null||D.addEventListener("click",()=>{ge()}),p==null||p.addEventListener("click",()=>{j(!A,`Shortcut help ${A?"closed":"opened"}.`,{moveFocus:!0})}),b==null||b.addEventListener("click",()=>{j(!1,"Shortcut help closed.",{moveFocus:!0})});function ne(n){if(!A||fe(n.target))return;const r=me(n.target),u=!!(m!=null&&m.contains(a.activeElement));j(!1,"Shortcut help closed.",{moveFocus:!r&&u})}function oe(n){var B;if(n.key==="Escape"&&S){n.preventDefault(),E(),C(),y("Template merge cancelled.");return}if(n.key==="Escape"&&A){n.preventDefault(),j(!1,"Shortcut help closed.",{moveFocus:!0});return}if(n.key==="Tab"&&A){const P=[h,b].filter(Boolean);if(!P.length)return;const V=e.ownerDocument.activeElement,f=P.indexOf(V);if(f===-1){n.preventDefault(),he(n.shiftKey);return}const w=n.shiftKey?(f-1+P.length)%P.length:(f+1)%P.length;n.preventDefault(),(B=P[w])==null||B.focus();return}if(n.key==="?"&&!ue(n.target)){n.preventDefault(),j(!A,`Shortcut help ${A?"closed":"opened"}.`,{moveFocus:!0});return}if(!n.altKey||n.ctrlKey||n.metaKey||n.shiftKey)return;const r=Number.parseInt(n.key,10)-1;if(Number.isNaN(r)||r<0||r>=q.length)return;n.preventDefault();const u=q[r];Q(u,{announcement:`Timeline filter set to ${g(u)}.`})}a.addEventListener("click",ne),a.addEventListener("keydown",oe);async function ie(n){if(!n)return;if(S){y("Resolve template merge before sending.");return}if(typeof t.performAction!="function"){M("Adapter: action unavailable"),y(`${g(n)} action is unavailable in the current adapter.`);return}const r=typeof(c==null?void 0:c.value)=="string"?c.value.trim():"";U(!0),M(`Adapter: sending ${n}`),y(`Sending ${g(n)} action.`);try{await t.performAction(n,r||void 0),E(),v=k(n),c&&(c.value=""),await G(),y(`${g(n)} action sent.`)}catch(u){o.innerHTML=re(u),M("Adapter: action failed",{degraded:!0}),U(!1),y(`${g(n)} action failed.`)}}c==null||c.addEventListener("keydown",n=>{if(n.key==="Escape"&&S){n.preventDefault(),E(),C(),y("Template merge cancelled.");return}if(n.key==="Enter"&&(n.ctrlKey||n.metaKey)&&!K(v)){n.preventDefault(),y(Fe(v));return}n.key==="Enter"&&(n.ctrlKey||n.metaKey)&&K(v)&&(n.preventDefault(),ie(v))});function se(){if(!W){J(),C();return}o.innerHTML=Ge(W,v),J(),C(),[...o.querySelectorAll('[data-role="timeline-filter"]')].forEach(r=>{r.addEventListener("click",()=>{const u=k(r.getAttribute("data-filter-kind"));Q(u,{announcement:`Timeline filter set to ${g(u)}.`})})})}async function G(){U(!0),s.textContent="Refreshing...",M("Adapter: syncing companion state");try{const n=await t.getCompanionState(),r=pe(n);W=r,se(),M(`Adapter: ${r.adapterLabel}`,{degraded:r.degraded})}catch(n){W=null,o.innerHTML=re(n),M("Adapter: degraded",{degraded:!0})}finally{U(!1),s.textContent="Refresh mood"}}for(const n of l)n.addEventListener("click",async()=>{const r=n.getAttribute("data-action");await ie(r)});s.addEventListener("click",()=>{G()});function Z(){a.removeEventListener("click",ne),a.removeEventListener("keydown",oe),e[O]===Z&&delete e[O]}return e[O]=Z,J(),ae(),C(),await G(),{destroy:Z,reload:G}}const Y=document.getElementById("app");Y&&Je(Y,{adapter:Ce()}).catch(e=>{Y.innerHTML=`
      <main class="companion-shell" data-surface="pet-companion">
