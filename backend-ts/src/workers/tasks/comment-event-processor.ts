/**
 * Comment event job processor — the worker's processJob body.
 * Extracted from comment-event.task.ts (pure extraction, no behavior change).
 *
...
import {
  buildInteractionEventFromPayload,
  buildPublishIntent,
  type CommentEventPayload,
} from './comment-event-task-helpers.js';

export async function processCommentEventJob(job: Job<CommentEventPayload>, services: WorkerServices) {
