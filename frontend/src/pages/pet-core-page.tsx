      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="rounded-lg border p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <StatCard label="关系等级" value={String(relationship.level || '-')} />
            <StatCard label="当前阶段" value={String(progress.progressLabel || '-')} />
          </div>
        ...
        <div className="rounded-lg border p-6 space-y-4">
          ...
          {needs.length === 0 ? (
            <div className="text-sm text-muted-foreground">暂无 needs 数据</div>
          ) : (
            <div className="grid grid-cols-2 gap-4">
              {needs.map((n, i) => (
                <StatCard key={i} label={String(n.label || n.key || 'Need')} value={String(n.value || '-')} />
              ))}
            </div>
          )}
        </div>
        ...
        {/* Companion Summary */}
        <div className="rounded-lg border p-6 space-y-4">
          <h3 className="font-semibold">Companion 摘要</h3>
          <div className="grid grid-cols-3 gap-4">
            <StatCard label="宠物名" value={String(companion.petName || '-')} />
            <StatCard label="Loop mode" value={String(companion.loopMode || '-')} />
            <StatCard label="状态来源" value={String(companion.adapterLabel || '-')} />
          </div>
        ...
        {/* Loop Actions */}
        <div className="rounded-lg border p-6 space-y-4">
          <h3 className="font-semibold">Loop 动作</h3>
          <p className="text-sm text-muted-foreground">直接记录 Pat / Feed / Wake，验证 pet loop 是否仍能持续推进。</p>
          <Textarea
            rows={3}
            maxLength={160}
